...
class ValidasiController extends CI_Controller {
	...
	function ValidasiIzin()
	{
		try {
            ...
            if ($savepemohon) {
          	     $slf = $this->saveSlf($savepemohon,$json);
                if ($slf) {
                    $KondisiSlf = $this->saveKondisiSlf($slf,$json);
                    if ($KondisiSlf) {
                            $iuts = $this->saveIuts($savepemohon,$slf,$json);
                            if ($iuts) {
	...
	function savePemohon($data)
	{
		...
		if ($cek->num_rows() > 0) {
			...
			$uuid = $this->uuid->v4();
        	$id = str_replace('-', '', $uuid);
			$q = $this->us->InsertPemohon($id,$namaLengkap,$jabatan,$nomorInKepen,$nomorInBeru,$npwp,$alamat_perusahaan,$no_telp,$emailAktif,$status_pemohon,$token);
		}
